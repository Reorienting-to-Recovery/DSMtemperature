...
# try other temps for model
stan_water <- rpn_water_temp
stan_air_temp <- cimis_71
...
# Trying CIMIS station 71 instead of modesto airport
cimis_71 <- read_csv("~/Downloads/cimis_71_full.csv") |>
  janitor::clean_names() |>
  select(date, mean_air_temp_c = avg_air_temp_c) |>
  filter(!is.na(date)) |> glimpse()
cimis_71 |>
...
cimis_71_air_temp <- cimis_71 |>
    bind_rows(
      tibble(date = seq.Date(ymd('1990-01-01'), ymd('2000-12-01'), by = 'month'),
             mean_air_temp_c = 0)
    ) %>%
    group_by(date) %>%
...
ts_cimis <- ts(cimis_71_air_temp$mean_air_temp_c, start = c(1990, 1), end = c(2000, 12), frequency = 12)
ts_cimis

na.interp(ts_cimis) %>% autoplot(series = 'Interpolated') +
  forecast::autolayer(ts_cimis, series = 'Original')

stanislaus_air_temp_c <- tibble(
  date = seq.Date(ymd('1990-01-01'), ymd('2000-12-01'), by = 'month'),
  mean_air_temp_c = as.numeric(na.interp(ts_cimis)))
